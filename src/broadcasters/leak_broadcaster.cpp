...
controller_interface::CallbackReturn LeakBroadcaster::on_init()
{
  try {
    ...
    auto_declare<std::string>("frame_id", "leak_sensor");
    auto_declare<std::string>("topic_name", "/sura/sensors/leak");
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  RCLCPP_INFO(
    get_node()->get_logger(),
    "Configured LeakBroadcaster: sensor_name='%s', frame_id='%s', topic='%s'",
    sensor_name_.c_str(),
    frame_id_.c_str(),
    topic_name_.c_str());
  ...
  const rclcpp::Duration &)
{
  ...
  msg.data = state_interfaces_[0].get_value() > 0.5;
