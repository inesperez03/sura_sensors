#include "sura_sensors/broadcasters/dvl75_gps_broadcaster.hpp"
...
controller_interface::CallbackReturn Dvl75GpsBroadcaster::on_init()
{
  try {
...
Dvl75GpsBroadcaster::command_interface_configuration() const
{
  return {controller_interface::interface_configuration_type::NONE};
}

controller_interface::InterfaceConfiguration
Dvl75GpsBroadcaster::state_interface_configuration() const
{
...
controller_interface::CallbackReturn Dvl75GpsBroadcaster::on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  position_covariance_ =
    get_node()->get_parameter("position_covariance").as_double();
...
controller_interface::CallbackReturn Dvl75GpsBroadcaster::on_activate(
  const rclcpp_lifecycle::State &)
{
  if (!publisher_) {
    return controller_interface::CallbackReturn::ERROR;
...
controller_interface::CallbackReturn Dvl75GpsBroadcaster::on_deactivate(
  const rclcpp_lifecycle::State &)
{
  if (publisher_) {
...
controller_interface::return_type Dvl75GpsBroadcaster::update(
...
PLUGINLIB_EXPORT_CLASS(
  sura_sensors::Dvl75GpsBroadcaster,
